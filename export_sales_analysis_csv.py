#!/usr/bin/env python3
"""
seed_and_export_textiles.py
- Creates SQLite DB: textiles.db
- Seeds master data, products, variants, sales
- Fetches holidays
- Generates expert-level sales CSV in 'sales/' folder
"""
...
DB = "textiles.db"
SALES_FOLDER = "sales"
EXPERT_CSV = os.path.join(SALES_FOLDER, "expert_sales_data.csv")
SEED = 42
random.seed(SEED)
        ...
        try:
            ...
            if "response" in j:
                for h in j["response"]["holidays"]:
                    ...
                    name = h["name"]
                    holidays[date] = name
            print(f"Fetched {len(holidays)} holidays for {y}")
...
def seed_master_data(conn):
    cur = conn.cursor()
    categories = ["Shirts","T-Shirts","Jeans","Trousers","Sarees","Dresses","Sweaters",
    ...
    cur.executemany("INSERT INTO suppliers (supplier_name, brand_name, contact_info) VALUES (?, ?, ?)", suppliers)
...
# Products & variants
# ---------------------------
def seed_products_and_variants(conn, num_products, category_brand_map):
    cur = conn.cursor()
    ...
    for pid in product_ids:
        ...
        if combos:
            ...
            cur.executemany("INSERT INTO product_variants (product_id, color_id, size_id, stock_quantity) VALUES (?, ?, ?, ?)", chosen)
    conn.commit()
...
def generate_sales(conn, holidays):
    ...
    while current_date<=END_DATE:
        ...
        if current_date.month in (10,11,12): mult*=1.2
    ...
    conn.commit()
    print(f"Sales generation complete. Total rows: {rows_inserted}")
...
def store_holidays_table(conn, holidays):
    ...
    conn.commit()
    print(f"Stored {len(rows)} holidays.")

# ---------------------------
# Expert CSV generation
# ---------------------------
def generate_expert_csv(conn):
    ...
    SELECT 
        ...
        m.material_name,
        sup.supplier_name,
        sup.contact_info AS supplier_contact,
        col.collection_name,
        GROUP_CONCAT(st.style_name) AS styles,
        v.variant_id,
        clr.color_name,
        sz.size_name,
        v.stock_quantity,
    ...
    LEFT JOIN product_styles ps ON p.product_id = ps.product_id
    LEFT JOIN styles st ON ps.style_id = st.style_id
    GROUP BY s.sale_id
    ...
    # Rolling & cumulative
    df['7d_qty'] = df.groupby('product_id')['sold_quantity'].transform(lambda x: x.rolling(7, min_periods=1).sum())
    df['30d_qty'] = df.groupby('product_id')['sold_quantity'].transform(lambda x: x.rolling(30, min_periods=1).sum())
    df['7d_revenue'] = df.groupby('product_id')['total_price'].transform(lambda x: x.rolling(7, min_periods=1).sum())
    df['30d_revenue'] = df.groupby('product_id')['total_price'].transform(lambda x: x.rolling(30, min_periods=1).sum())
    df['cumulative_qty'] = df.groupby('product_id')['sold_quantity'].cumsum()
    df['cumulative_revenue'] = df.groupby('product_id')['total_price'].cumsum()
    # Discount info
    df['discount_percent'] = df['offer'].fillna(0)
    df['discount_value'] = df['unit_price'] - df['price_after_offer']
    df.to_csv(EXPERT_CSV, index=False)
    print(f"✅ Expert CSV saved: {EXPERT_CSV} Rows: {len(df)} Columns: {len(df.columns)}")
...
def main():
    ...
    holidays = fetch_holidays_calendarific(years)
    store_holidays_table(conn, holidays)
    generate_sales(conn, holidays)
    generate_expert_csv(conn)
    conn.close()
    print("✅ All done. Database & expert CSV ready.")
